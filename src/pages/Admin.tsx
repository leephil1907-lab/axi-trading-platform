  ...
  return (
    <div className="min-h-[calc(100vh-4rem)] bg-[#f4f5f7] text-[#171717] dark:bg-[#0b0d10] dark:text-white">
      <div className="border-b border-[#dedede] bg-white dark:border-white/10 dark:bg-[#111419]">
        <div className="mx-auto flex min-h-[74px] max-w-[1320px] items-center justify-between gap-4 px-5 lg:px-8">
          <div className="flex min-w-0 items-center gap-3 sm:gap-5">
            <Link to="/trading" className="inline-flex items-center gap-2 text-sm font-semibold text-[#343434] hover:text-[#c8102e] dark:text-white/80 dark:hover:text-white">
              <ArrowLeft className="h-4 w-4" />
              <span className="hidden sm:inline">Trading</span>
            </Link>
            <span className="h-6 w-px bg-[#d9d9d9] dark:bg-white/10" />
            <div className="flex min-w-0 items-center gap-2.5">
              <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full border-2 border-[#c8102e] text-[#c8102e]"><ShieldCheck className="h-4 w-4" /></span>
              <span className="truncate text-base font-extrabold tracking-[-0.02em] sm:text-lg">Admin Dashboard</span>
            </div>
            <span className="hidden items-center gap-1.5 rounded-full bg-[#e8f7ed] px-3 py-1.5 text-xs font-bold text-[#18743c] sm:inline-flex dark:bg-emerald-500/10 dark:text-emerald-300">
              <span className="h-2 w-2 rounded-full bg-[#1c9a50]" />
              System Online
            </span>
          </div>

          <div className="flex items-center gap-2">
            <button onClick={logout} className="hidden items-center gap-2 rounded-lg border border-[#d9d9d9] bg-white px-4 py-2.5 text-sm font-bold hover:bg-[#f7f7f7] sm:inline-flex dark:border-white/15 dark:bg-[#15191f] dark:hover:bg-white/5">
              <LogOut className="h-4 w-4" /> Sign out
            </button>
            <button onClick={() => setMobileNavOpen((open) => !open)} aria-label="Toggle admin navigation" className="inline-flex rounded-lg border border-[#ddd] p-2 sm:hidden dark:border-white/15">
      ...
      <main className="mx-auto max-w-[1320px] px-5 py-7 lg:px-8 lg:py-8">
        ...
        {mobileNavOpen && (
          <div className="mt-4 rounded-2xl border border-[#ddd] bg-white p-3 shadow-sm dark:border-white/10 dark:bg-[#111419] sm:hidden">
            <AdminTabList activeTab={activeTab} selectTab={selectTab} />
            <button onClick={logout} className="mt-2 flex w-full items-center gap-2 rounded-xl px-3 py-3 text-left text-sm font-bold text-[#c8102e] hover:bg-red-50 dark:hover:bg-red-500/10"><LogOut className="h-4 w-4" /> Sign out</button>
          </div>
        )}

        <div className="mt-7 hidden overflow-x-auto border-b border-[#d8d8d8] md:block dark:border-white/10">
          <AdminTabList activeTab={activeTab} selectTab={selectTab} horizontal />
        </div>
        ...
        <section className="mt-6 overflow-hidden rounded-2xl border border-[#dfdfdf] bg-white shadow-[0_1px_3px_rgba(0,0,0,.03)] dark:border-white/10 dark:bg-[#111419]">
          <div className="border-b border-[#e6e6e6] px-6 py-5 dark:border-white/10 sm:px-7">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 text-[#c8102e]"><Activity className="h-5 w-5" /><span className="text-base font-extrabold">{active.id === "overview" ? "Operations overview" : active.label}</span></div>
                <p className="mt-1.5 max-w-3xl text-sm leading-6 text-[#777] dark:text-white/55">{descriptionFor(activeTab)}</p>
              </div>
              <span className="hidden rounded-full bg-[#f1f7f3] px-3 py-1 text-xs font-bold text-[#197541] sm:inline-flex dark:bg-emerald-500/10 dark:text-emerald-300">Production controls</span>
            </div>
          </div>

          {activeTab === "overview" ? (
            <OverviewContent wallets={wallets} onSelect={selectTab} />
          ) : (
            <ManagementPanel tab={activeTab} wallets={wallets} accountCount={accountCount} onSelect={selectTab} />
          )}
        </section>

        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 text-xs text-[#888] dark:text-white/45">
          <span>Authenticated operator: <strong className="text-[#555] dark:text-white/70">{user?.email || "—"}</strong></span>
          <Link to="/trading" className="inline-flex items-center gap-1 font-bold text-[#c8102e] hover:underline">Open client trading area <ExternalLink className="h-3.5 w-3.5" /></Link>
        </div>
      </main>
    </div>
  )
...
function MetricCard({ icon, iconClass, badge, value, label }: { icon: React.ReactNode; iconClass: string; badge: string; value: string; label: string }) {
  return (
    <div className="min-h-[162px] rounded-2xl border border-[#dfdfdf] bg-white p-5 shadow-[0_1px_3px_rgba(0,0,0,.025)] dark:border-white/10 dark:bg-[#111419]">
      <div className="flex items-start justify-between gap-3">
        <span className={`grid h-12 w-12 place-items-center rounded-xl text-white ${iconClass}`}>{icon}</span>
        <span className="rounded-full bg-[#edf8f1] px-2.5 py-1 text-[11px] font-bold text-[#247043] dark:bg-emerald-500/10 dark:text-emerald-300">{badge}</span>
      </div>
      <div className="mt-4 text-3xl font-black tracking-tight">{value}</div>
      <div className="mt-1 text-sm font-semibold text-[#777] dark:text-white/55">{label}</div>
    </div>
  )
}
...
function AdminTabList({ activeTab, selectTab, horizontal = false }: { activeTab: TabId; selectTab: (id: TabId) => void; horizontal?: boolean }) {
  return (
    <nav className={horizontal ? "flex min-w-max items-center gap-1" : "space-y-1"} aria-label="Admin sections">
      {tabs.map(({ id, label, icon: Icon }) => (
        <button key={id} onClick={() => selectTab(id)} className={horizontal ? `relative inline-flex items-center gap-2 px-4 py-3 text-sm font-bold ${activeTab === id ? "text-[#202020] after:absolute after:bottom-[-1px] after:left-2 after:right-2 after:h-[3px] after:bg-[#202020] dark:text-white dark:after:bg-white" : "text-[#999] hover:text-[#333] dark:text-white/45 dark:hover:text-white/80"}` : `flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left text-sm font-bold ${activeTab === id ? "bg-[#f7f7f7] text-[#c8102e] dark:bg-white/5" : "text-[#777] hover:bg-[#f7f7f7] dark:text-white/55 dark:hover:bg-white/5"}`}>
  ...
  return (
    <div className="p-6 sm:p-7">
      <div className="grid gap-3 lg:grid-cols-3">
        <QuickAction icon={<CreditCard className="h-5 w-5" />} title="Review funding" description="Open the persisted funding queue" onClick={() => onSelect("funding")} />
        <QuickAction icon={<Users className="h-5 w-5" />} title="Manage users" description="Accounts, balances and controls" onClick={() => onSelect("users")} />
        <QuickAction icon={<CheckCircle2 className="h-5 w-5" />} title="Review KYC" description="Open verification management" onClick={() => onSelect("kyc")} />
      </div>

      <div className="mt-7">
        <div className="mb-3 flex items-center justify-between"><h3 className="text-base font-extrabold">Recent activity</h3><button onClick={() => onSelect("activity")} className="text-sm font-extrabold text-[#c8102e] hover:underline">View all</button></div>
        <div className="overflow-x-auto rounded-xl border border-[#e5e5e5] dark:border-white/10">
          <div className="grid min-w-[720px] grid-cols-[1.1fr_1.3fr_1.5fr_1fr] bg-[#f8f7f2] px-4 py-3 text-[11px] font-extrabold uppercase tracking-[0.12em] text-[#858585] dark:bg-white/5">
            <span>Actor</span><span>Action</span><span>Target</span><span>When</span>
          </div>
          <div className="flex min-h-[108px] items-center justify-center px-5 text-center text-sm text-[#8a8a8a] dark:text-white/45">
            No persisted admin activity is available yet. Actions will appear here when connected to the production audit log.
          </div>
        </div>
      </div>

      <div className="mt-7 rounded-xl border border-dashed border-[#d6d6d6] p-5 dark:border-white/10">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div><p className="font-extrabold">Wallet configuration</p><p className="mt-1 text-sm text-[#777] dark:text-white/50">{wallets.length ? `${wallets.length} configured crypto network${wallets.length === 1 ? "" : "s"}.` : "No deposit addresses are configured yet."}</p></div>
          <button onClick={() => onSelect("wallets")} className="inline-flex items-center gap-2 text-sm font-extrabold text-[#c8102e]">Manage wallets <ChevronRight className="h-4 w-4" /></button>
        </div>
      </div>
    </div>
  )
...
function ManagementPanel({ tab, wallets, accountCount, onSelect }: { tab: Exclude<TabId, "overview">; wallets: ReturnType<typeof configuredCryptoNetworks>; accountCount: number; onSelect: (id: TabId) => void }) {
  ...
  if (tab === "pnl") return <div className="p-6 sm:p-7"><EmptyState title="P&L control" text="No fabricated P&L controls are shown. Connect the live trading ledger before exposing balance or profit adjustments." /></div>
